...
class archiveImporterXmlNative
{
  ...
  public function __construct($title = "Вестник МГСУ", $abstract = 'Научно-технический журнал по строительству и архитектуре')
  {
  ...
  public function importIssue($doi_settings = array('core' => '10.22227', 'issn' => '1997-0935', 'useNum' => '1', 'x' => 0, 'useVolume' => 0, 'useYear' => 1, 'useDoi' => 1))
  {
  ...
  {
    $doi = $doi_settings['core'] . '/' . $doi_settings['issn'];
  ...
  {
    //маркеры для определения отдельных элементов статьи
    ...
    $element_path = array(
        ...
        //*[contains(@class,"abstract_abstract-ru") and not(contains(.,"АННОТАЦИЯ")) and not(contains(.,"Ключевые слова:")) and not(contains(.,"ДЛЯ ЦИТИРОВАНИЯ:"))] |
        ...
        //*[contains(@class,"abstract_abstract-en") and not(contains(.,"ABSTRACT")) and not(contains(.,"Keywords:")) and not(contains(.,"FOR CITATION:"))] |
     ...
     */
      ...
      foreach ( $temp as $key => $item)
      {
        ...
        $additional_data = trim(str_replace(
          ...
          '',
          (string) array_shift(
            $this->xml->xpath(str_replace('#surname_placeholder#',
            $surname_paceholder,
